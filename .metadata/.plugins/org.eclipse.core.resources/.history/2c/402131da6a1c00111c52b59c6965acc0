...
import com.trendify.model.*;
import com.trendify.repository.*;
...
public class OrderService {
    ...
    public Order placeOrder(Long userId, OrderRequest request) {
        ...
        for (OrderRequest.OrderItemRequest itemReq : request.getItems()) {
            ...
            product.setTotalSales(product.getTotalSales() + itemReq.getQuantity());
    ...
    public Order getOrderByNumber(String orderNumber, Long userId) {
        Order order = orderRepository.findByOrderNumber(orderNumber)
                .orElseThrow(() -> new ResourceNotFoundException("Order not found: " + orderNumber));
        if (!order.getUser().getId().equals(userId)) {
            throw new SecurityException("Access denied to order: " + orderNumber);
        }
        return order;
    }
    ...
    private String generateOrderNumber() {
        ...
        String randomPart = String.valueOf(new Random().nextInt(90000) + 10000);
